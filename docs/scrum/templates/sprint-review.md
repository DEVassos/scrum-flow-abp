<!-- Template — copie para seu destino e ajuste o caminho do Índice -->
← [Índice da Documentação](../../README.md)

# Sprint Review — Sprint X

**Data:** DD/MM/YYYY  
**Horário:** HH:MM às HH:MM  
**Scrum Master:** Gabriel Travensolli  
**Product Owner:** Gustavo Koiti  
**Presentes:** Gabriel Travensolli, Gustavo Koiti, Andrea Turíbio, Henrique Camargo, Lucas Amorim, Marcello Campbell, Vinicius Augusto

---

## Meta da Sprint

> Repita aqui a meta definida na Sprint Planning.

---

## Itens Entregues (Conforme DoD)

| ID | História | Status | Observações |
|----|----------|--------|-------------|
| US0X | — | ✅ Entregue / ❌ Não entregue | — |

**Pontos entregues:** — / **Pontos planejados:** —

---

## Demonstração do Incremento

> Descreva o que foi demonstrado ao Product Owner e demais presentes.

_Exemplo: "Foi demonstrado o fluxo completo de cadastro e login, com validação de CPF duplicado e hash de senha."_

---

## Feedback do Product Owner

| Item | Feedback | Ação Necessária |
|------|----------|-----------------|
| — | — | — |

---

## Itens Não Entregues

| ID | História | Motivo | Destino (próxima sprint / backlog) |
|----|----------|--------|------------------------------------|
| — | — | — | — |

---

## Observações Finais

_Registre aqui qualquer ponto relevante da revisão._

